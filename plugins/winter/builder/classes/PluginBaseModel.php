...
class PluginBaseModel extends PluginYamlModel
{
    ...
    protected function modelToYamlArray()
    {
        ...
        foreach ($attributes as $attribute) {
            if ($attribute === 'replaces') {
                ...
                // Ensure $this->replaces is an array before iterating
                $replacesData = $this->{$attribute};
                if (is_array($replacesData) && !empty($replacesData)) {
                    foreach ($replacesData as $replace) {
                        if (is_array($replace) && isset($replace['plugin_code']) && isset($replace['version_constraint'])) {
    ...
    protected function yamlArrayToModel($array)
    {
        ...
        // Safely handle replaces data
        $replacesData = $this->getArrayKeySafe($array, 'replaces', []);
        $this->replaces = [];
        ...
        if (is_array($replacesData) && !empty($replacesData)) {
            $this->replaces = array_map(
                ...
                array_keys($replacesData),
                array_values($replacesData)
            );
        }
    }
